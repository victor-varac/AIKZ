...
import { getInventarioPolietileno, getNotasVentaPolietileno, getEntradasPolietilenoPorProducto, entregarTodoPolietileno, registrarEntregasParcialesPolietileno, createMovimientoPolietileno } from '../../services/api/almacen';
import InventarioPolietilenoTable from './InventarioPolietilenoTable';
import EntregasPolietilenoCards from './EntregasPolietilenoCards';
import EntradasPolietilenoCards from './EntradasPolietilenoCards';
import EntregaParcialModal from './EntregaParcialModal';
import MovimientoPolietilenoForm from './MovimientoPolietilenoForm';
import ProductoPolietilenoDetail from './ProductoPolietilenoDetail';
import EditarEntregasModal from './EditarEntregasModal';
import AjusteInventarioPolietilenoModal from './AjusteInventarioPolietilenoModal';

const AlmacenPolietilenoList = () => {
  ...
  const loadData = async () => {
    try {
      ...
      if (view === 'inventario') {
        const data = await getInventarioPolietileno();
        setInventario(data);
      } else if (view === 'entregas') {
        const data = await getNotasVentaPolietileno();
        setNotasVenta(data);
  ...
  const handleEntregarTodo = async (nota) => {
    try {
      const confirmacion = window.confirm(
        ...
        `Total a entregar: ${(nota.estadisticas?.totalPendiente || 0).toLocaleString()} kg\n\n` +
      ...
      setLoading(true);
      await entregarTodoPolietileno(nota.id);
  ...
  const handleAjusteCreated = async (ajusteData) => {
    try {
      ...
      await createMovimientoPolietileno(ajusteData);
      ...
      alert(`✅ Ajuste de inventario completado:\n\nProducto: ${producto.producto}\nStock anterior: ${producto.stock_kilos} kg\nMovimiento: ${tipoAjuste} ${ajusteData.kilos} kg\n\nEl inventario ha sido actualizado.`);
  ...
  const handleEntregaRegistrada = async (entregas) => {
    try {
      await registrarEntregasParcialesPolietileno(entregas);

      // Recargar datos para reflejar los cambios
      await loadData();

      const totalEntregado = entregas.reduce((sum, entrega) => sum + entrega.cantidad, 0);
      alert(`✅ Entrega parcial registrada: ${totalEntregado.toLocaleString()} kg`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                🛢️ Almacén de Polietileno
              </h1>
              <p className="mt-2 text-gray-600">
                Gestión de inventario y movimientos de productos de polietileno (en kilogramos)
              </p>
            </div>
        ...
        <AjusteInventarioPolietilenoModal
...
export default AlmacenPolietilenoList;