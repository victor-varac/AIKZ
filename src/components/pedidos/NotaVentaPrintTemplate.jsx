...
const NotaVentaPrintTemplate = React.forwardRef(({ notaVenta }, ref) => {
  ...
  if (!notaVenta) {
    return (
      <div ref={ref} style={{ padding: '20px', backgroundColor: 'white', minHeight: '200px' }}>
        <h1>No hay datos de la nota de venta para imprimir</h1>
      </div>
    );
  }
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: '30px', borderBottom: '2px solid #333', paddingBottom: '15px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: '0 0 10px 0' }}>AIKZ INDUSTRIAL</h1>
      ...
      <div style={{ marginBottom: '25px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #333' }}>
          ...
          <tbody>
            {notaVenta.pedidos?.map((pedido, index) => (
              <tr key={pedido.id || index}>
                ...
                <td style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
                  {pedido.cantidad?.toLocaleString('es-MX') || '0'}
                </td>
              ...
              <tr>
                <td colSpan="3" style={{ border: '1px solid #333', padding: '8px', textAlign: 'center' }}>
      ...
      <div style={{ marginBottom: '25px', textAlign: 'right' }}>
        <div style={{ backgroundColor: '#f5f5f5', padding: '15px', borderRadius: '5px', display: 'inline-block', minWidth: '250px' }}>
          <div style={{ marginBottom: '10px' }}>
            <strong>Subtotal: {formatCurrency(notaVenta.subtotal)}</strong>
          </div>
          {notaVenta.descuento > 0 && (
            <div style={{ marginBottom: '10px', color: 'red' }}>
              Descuento ({notaVenta.descuento}%): -{formatCurrency((notaVenta.subtotal * notaVenta.descuento) / 100)}
            </div>
          )}
          <div style={{ marginBottom: '10px' }}>
            <strong>IVA (16%): {notaVenta.iva > 0 ? formatCurrency(notaVenta.iva) : 'Sin IVA'}</strong>
          </div>
          <div style={{ borderTop: '1px solid #333', paddingTop: '10px', fontSize: '16px' }}>
            <strong>Total: {formatCurrency(notaVenta.total)}</strong>
          </div>
        </div>
      </div>
...
NotaVentaPrintTemplate.displayName = 'NotaVentaPrintTemplate';

export default NotaVentaPrintTemplate;